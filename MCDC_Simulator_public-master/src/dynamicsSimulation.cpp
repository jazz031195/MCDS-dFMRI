...
void DynamicsSimulation::getAnExtraCellularPositionOnEdge(Vector3d &extra_pos, double z)
{
    ...
    while(true){
        ...
        if(checkIfPosInsideVoxel(pos_temp) && (!isInIntra(pos_temp, dummy_a,dummy_b,dummy_c,dummy_d,dummy_d,barrier_tickness))){
            extra_pos = pos_temp;
            return;
        }
        count++;
...
bool DynamicsSimulation::checkObstacleCollision(Vector3d &bounced_step, double &tmax, Vector3d& end_point, Collision& colision)
{
...
bool DynamicsSimulation::updateWalkerPositionAndHandleBouncing(Vector3d &bounced_step, double &tmax, Collision &colision)
{
    ...
    else if(colision.type == Collision::hit && colision.col_location == Collision::voxel)
    {
        ...
        else{            mapWalkerIntoVoxel(bounced_step, colision, barrier_tickness);
        }
        bounced_step = colision.bounced_direction;
        tmax        -= colision.t;
    }
