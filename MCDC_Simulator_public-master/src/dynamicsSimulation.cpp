...
{
    int cyl_id, ply_id, ax_id, neuron_id;
    ...
    if (sentinela.deport_illegals and params.obstacle_permeability <= 0 and walker.status != Walker::bouncing and walker.status != Walker::on_object)
    {
        ...
        bool isIntra = isInIntra(this->walker.pos_v, cyl_id, ply_id, sph_id, ax_id, neuron_id, barrier_tickness);
        ...
        if ((isIntra and this->walker.initial_location == Walker::extra) or ((!isIntra and this->walker.initial_location == Walker::intra)))
        {
            ...
            // cout << isIntra << endl;
            // TODO check this [ines]
            cout << " Walker current location is not matching Walker.initial_location." << endl;
            return true;
        }
    }
    return false;
...
void DynamicsSimulation::iniWalkerPosition()
{
    ...
    else if (voxels_list.size() > 0 or params.custom_sampling_area)
    {
        ...
        {
            bool intra_flag = isInIntra(walker.ini_pos, walker.in_cyl_index, walker.in_ply_index, walker.in_sph_index,
                                        walker.in_ax_index, walker.in_neuron_index, 0.0);
            walker.location = (intra_flag == 1) ? Walker::RelativeLocation::intra : Walker::RelativeLocation::extra;
            walker.initial_location = walker.location;
        }
    }
...
void DynamicsSimulation::initWalkerObstacleIndexes()
{
    ...
    // std::cout << outer_col_dist_factor << endl;
    // std::cout << step_lenght << endl;
    // std::cout << "--------"<< endl;
...
void DynamicsSimulation::getAnIntraCellularPosition(Vector3d &intra_pos, int &cyl_ind, int &ply_ind, std::vector<int> &sph_ind, int &ax_ind, int &neuron_ind)
{
    string message;
    bool isintra = false;
    std::random_device rd;
...
void DynamicsSimulation::getAnIntraCellularPositionOnEdge(Vector3d &intra_pos, int &cyl_ind, int &ply_ind, std::vector<int> &sph_ind, int &ax_ind, int &neuron_ind, double z)
{
    ...
    if (cylinders_list->size() <= 0 and dyn_cylinders_list->size() <= 0 and axons_list->size() <= 0 and plyObstacles_list->size() <= 0 and dyn_spheres_list->size() <= 0)
    {
        SimErrno::error("Cannot initialize intra-axonal walkers within the given substrate.", cout);
        SimErrno::error("There's no defined intra-axonal compartment (missing obstacles?)", cout);
        assert(0);
    ...
    while (!achieved)
    {
        ...
        isintra = isInIntra(pos_temp, cyl_ind, ply_ind, sph_ind, ax_ind, neuron_ind, 0);
        if (checkIfPosInsideVoxel(pos_temp) && (isintra))
        {
...
void DynamicsSimulation::getAnExtraCellularPositionOnEdge(Vector3d &extra_pos, double z)
{
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> udist(0, 1);
    int dummy_a, dummy_b, dummy_d;
    std::vector<int> dummy_c;
    ...
    while (true)
    {
        ...
        if (checkIfPosInsideVoxel(pos_temp) && (!isInIntra(pos_temp, dummy_a, dummy_b, dummy_c, dummy_d, dummy_d, 0)))
        {
            extra_pos = pos_temp;
            return;
        }
...
void DynamicsSimulation::getAnExtraCellularPosition(Vector3d &extra_pos)
{
    ...
    while (true)
    {
        ...
        if (checkIfPosInsideVoxel(pos_temp) && (!isInIntra(pos_temp, dummy_int, dummy_int, dummy_vect_int, dummy_int, dummy_int, barrier_tickness)))
        {
            extra_pos = pos_temp;
            return;
        }
...
bool DynamicsSimulation::isInsideAxons(Vector3d &position, int &ax_id, double distance_to_be_inside, std::vector<int> &col_sphere_ids)
{
...
bool DynamicsSimulation::isInIntra(Vector3d &position, int &cyl_id, int &ply_id, std::vector<int> &sph_id, int &ax_id, int &neuron_id, double const &distance_to_be_intra_ply)
{
    bool isIntra = false;
    total_tries++;
    ...
    {
        isIntra |= this->isInsideAxons(position, ax_id, barrier_tickness, sph_id);
    }
...
void DynamicsSimulation::startSimulation(SimulableSequence *dataSynth)
{
    ...
    for (w = 0; w < params.num_walkers; w++)
    {
        ...
        for (unsigned t = 1; t <= params.num_steps; t++) // T+1 steps in total (avoid errors)
        {
            ...
            // cout << "update pos" << walker.pos_v[0] << " " << walker.pos_v[1] << " " << walker.pos_v[2] << endl;
        ...
        // Write the positions.
        trajectory.writePosition(walker.pos_v_log);
    ...
    if (params.verbatim)
        SimErrno::info(" Sim: " + to_string(id) + " Simulation ended after: " + secondsToMinutes(second_passed) + " seconds", cout);
 ...
 * @return void
 */
void DynamicsSimulation::generateStep(Vector3d &step)
{
    ...
    // It should change nothing to multiply by l and then normalize...
    step(0) = cosTh * sinPhi;
    step(1) = sinTh * sinPhi;
    step(2) = cosPhi;
...
// vector<Dynamic_Sphere *> Neuron::find_neighbor_spheres(Walker &walker, Vector3d const &next_step, double const &step_length)
    ...
    int sphere_id = 0;
    Dynamic_Sphere soma(center, radius_soma, sphere_id);
    ...
    vector<Axon> dendrite({subbranch});
    Dendrite d(dendrite);
    Neuron n({d}, soma);
    ...
    struct DynamicsSimulationTest : DynamicsSimulation
    {
        ...
        {
            return DynamicsSimulation::updateWalkerPosition(step);
        }
    } simu;


    auto empty_vect = vector<unsigned int>();
    simu.walker.dyn_cylinders_collision_sphere.collision_list = &empty_vect;
    simu.walker.cylinders_collision_sphere.collision_list = &empty_vect;
    simu.walker.spheres_collision_sphere.collision_list = &empty_vect;
    simu.walker.dyn_spheres_collision_sphere.collision_list = &empty_vect;
    simu.walker.axons_collision_sphere.collision_list = &empty_vect;
    
    auto empty_vect_vect = vector<vector<unsigned int>>();
    simu.walker.ply_collision_sphere.collision_list = &empty_vect_vect;

    vector <PLYObstacle> plyObstacles_list;      
    vector <Cylinder> cylinders_list;            
    vector <Dynamic_Cylinder> dyn_cylinders_list;
    vector <Sphere> spheres_list;                
    vector <Dynamic_Sphere> dyn_spheres_list;    
    vector <Axon> axons_list;                    

    simu.plyObstacles_list = &plyObstacles_list;
    simu.cylinders_list = &cylinders_list;
    simu.dyn_cylinders_list = &dyn_cylinders_list;
    simu.spheres_list = &spheres_list;
    simu.dyn_spheres_list = &dyn_spheres_list;
    ...
    simu.step = direction;
    simu.step_lenght = step_length;
    simu.neurons_list = &neurons;
    simu.walker.in_neuron_index = 0;  // in the first (only) neuron
    simu.walker.in_dendrite_index = 0;
    simu.walker.in_subbranch_index = 0;
    simu.walker.location = simu.walker.initial_location = Walker::intra;  

    auto neur_list = vector<unsigned int>({0});
    simu.walker.neurons_collision_sphere.collision_list = &neur_list;
    simu.walker.neurons_collision_sphere.pushToBigSphere(0);
    simu.walker.neurons_collision_sphere.pushToSmallSphere(0);
    ...
    SUBCASE("trivial hit")
    {
        ...
        CHECK_EQ(simu.walker.last_pos_r, ini_pos);
        CHECK_EQ(simu.walker.last_pos_v, ini_pos);
...
{
    // assert((step.norm() - 1) < 1e-9 && "The norm of the step isn't 1.");
    // new step to take
    Vector3d bounced_step = step;
    Vector3d end_point;
    ...
    do
    {
        bounced = false;
        bouncing_count++;
        cout << "nb bouncing" << bouncing_count << endl;
        ...
        cout << "update_walker_status: " << update_walker_status << endl;
    ...
    if (tmax >= 0.0)
    {
        ...
        // int dummy_int;
        // vector<int> dummy_vect_int;
        // if(!isInIntra(walker.pos_v, dummy_int, dummy_int, dummy_vect_int, dummy_int, dummy_int))
...
bool DynamicsSimulation::checkObstacleCollision(const Vector3d &bounced_step, double &tmax, Vector3d &end_point, Collision &colision)
{
...
void DynamicsSimulation::mapWalkerIntoVoxel(Eigen::Vector3d &bounced_step, Collision &colision, double barrier_thicknes)
{
    ...
    if (mapped_on_z && walker.in_ax_index >= 0)
    {
        ...
        while (!incorrectcomp)
        {
            ...
            // TODO : change this so it's based on current walker relative location
...
bool DynamicsSimulation::isIntraEdge(Eigen::Vector3d &position, bool low_edge, int &ax_id, bool init_loc_intra)
{
    ...
    for (size_t i = 0; i < axons_list->size(); i++)
    {
        Vector3d sph_ref;
        // Walker goes out from the lower z plane => just check the first sphere
        if (low_edge)
        {
        ...
        // Check that the position is inside the first/last sphere of the axon
        double distance = (sph_ref - position).norm();
        if (distance < axons_list->at(i).radius)
        {
            // Check if it is inside voxel (should be useless)
...
void DynamicsSimulation::mapWalkerIntoVoxel_tortuous(Eigen::Vector3d &bounced_step, Collision &colision, double barrier_thicknes)
{
    ...
    if (z_mapped_min || z_mapped_max)
    {
        int cyl_ind, ply_ind, ax_id, neuron_id;
        ...
        {
            getAnIntraCellularPositionOnEdge(voxel_pos, cyl_ind, ply_ind, sph_ind, ax_id, neuron_id, z);
        }
    }
...
TEST_CASE("updateWalkerPositionAndHandleBouncing")
{
    struct DynamicsSimulationTest : DynamicsSimulation
    {
        ...
        {
            return DynamicsSimulation::updateWalkerPositionAndHandleBouncing(amended_step, tmax, colision);
        }
    } simu;
...
bool DynamicsSimulation::updateWalkerPositionAndHandleBouncing(Vector3d &bounced_step, double &tmax, Collision &colision)
{
    ...
    if (colision.type == Collision::hit && colision.col_location != Collision::voxel)
    {
        ...
        // tmax = step_length => tmax-displ => lenght of the reflected ray
        tmax -= displ;
        ...
        // We update the position. This is the position of the bounce.
        // bounced_step = step at this moment
    ...
    {
        walker.setLastCollision(colision.collision_objects);
    }
